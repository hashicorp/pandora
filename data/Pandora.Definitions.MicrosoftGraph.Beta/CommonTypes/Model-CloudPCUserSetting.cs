// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Pandora.Definitions.Attributes;
using Pandora.Definitions.Attributes.Validation;
using Pandora.Definitions.CustomTypes;


namespace Pandora.Definitions.MicrosoftGraph.Beta.CommonTypes;

internal class CloudPCUserSettingModel
{
    [JsonPropertyName("assignments")]
    public List<CloudPCUserSettingAssignmentModel>? Assignments { get; set; }

    [JsonPropertyName("createdDateTime")]
    public DateTime? CreatedDateTime { get; set; }

    [JsonPropertyName("displayName")]
    public string? DisplayName { get; set; }

    [JsonPropertyName("id")]
    public string? Id { get; set; }

    [JsonPropertyName("lastModifiedDateTime")]
    public DateTime? LastModifiedDateTime { get; set; }

    [JsonPropertyName("localAdminEnabled")]
    public bool? LocalAdminEnabled { get; set; }

    [JsonPropertyName("@odata.type")]
    public string? ODataType { get; set; }

    [JsonPropertyName("resetEnabled")]
    public bool? ResetEnabled { get; set; }

    [JsonPropertyName("restorePointSetting")]
    public CloudPCRestorePointSettingModel? RestorePointSetting { get; set; }

    [JsonPropertyName("selfServiceEnabled")]
    public bool? SelfServiceEnabled { get; set; }
}
