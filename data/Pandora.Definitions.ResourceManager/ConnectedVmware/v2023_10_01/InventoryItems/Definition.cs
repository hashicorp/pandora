using System.Collections.Generic;
using Pandora.Definitions.Interfaces;


// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.


namespace Pandora.Definitions.ResourceManager.ConnectedVmware.v2023_10_01.InventoryItems;

internal class Definition : ResourceDefinition
{
    public string Name => "InventoryItems";
    public IEnumerable<Interfaces.ApiOperation> Operations => new List<Interfaces.ApiOperation>
    {
        new CreateOperation(),
        new DeleteOperation(),
        new GetOperation(),
        new ListByVCenterOperation(),
    };
    public IEnumerable<System.Type> Constants => new List<System.Type>
    {
        typeof(InventoryTypeConstant),
        typeof(OsTypeConstant),
        typeof(ProvisioningStateConstant),
    };
    public IEnumerable<System.Type> Models => new List<System.Type>
    {
        typeof(ClusterInventoryItemModel),
        typeof(DatastoreInventoryItemModel),
        typeof(HostInventoryItemModel),
        typeof(InventoryItemModel),
        typeof(InventoryItemDetailsModel),
        typeof(InventoryItemPropertiesModel),
        typeof(ResourcePoolInventoryItemModel),
        typeof(VirtualMachineInventoryItemModel),
        typeof(VirtualMachineTemplateInventoryItemModel),
        typeof(VirtualNetworkInventoryItemModel),
    };
}
