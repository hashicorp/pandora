using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Pandora.Definitions.Attributes;
using Pandora.Definitions.Attributes.Validation;
using Pandora.Definitions.CustomTypes;


// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.


namespace Pandora.Definitions.ResourceManager.Workloads.v2023_04_01.SAPVirtualInstances;

[ValueForType("Discovery")]
internal class DiscoveryConfigurationModel : SAPConfigurationModel
{
    [JsonPropertyName("appLocation")]
    public string? AppLocation { get; set; }

    [JsonPropertyName("centralServerVmId")]
    public string? CentralServerVMId { get; set; }

    [JsonPropertyName("managedRgStorageAccountName")]
    public string? ManagedRgStorageAccountName { get; set; }
}
