using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Pandora.Definitions.Attributes;
using Pandora.Definitions.Attributes.Validation;
using Pandora.Definitions.CustomTypes;


// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.


namespace Pandora.Definitions.ResourceManager.HDInsight.v2021_06_01.Regions;


internal class AaddsResourceDetailsModel
{
    [JsonPropertyName("domainName")]
    public string? DomainName { get; set; }

    [JsonPropertyName("initialSyncComplete")]
    public bool? InitialSyncComplete { get; set; }

    [JsonPropertyName("ldapsEnabled")]
    public bool? LdapsEnabled { get; set; }

    [JsonPropertyName("ldapsPublicCertificateInBase64")]
    public string? LdapsPublicCertificateInBase64 { get; set; }

    [JsonPropertyName("resourceId")]
    public string? ResourceId { get; set; }

    [JsonPropertyName("subnetId")]
    public string? SubnetId { get; set; }

    [JsonPropertyName("tenantId")]
    public string? TenantId { get; set; }
}
