using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Pandora.Definitions.Attributes;
using Pandora.Definitions.Attributes.Validation;
using Pandora.Definitions.CustomTypes;


// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.


namespace Pandora.Definitions.ResourceManager.MachineLearningServices.v2023_04_01_preview.LabelingJob;

[ValueForType("Enabled")]
internal class MLAssistConfigurationEnabledModel : MLAssistConfigurationModel
{
    [JsonPropertyName("inferencingComputeBinding")]
    [Required]
    public string InferencingComputeBinding { get; set; }

    [JsonPropertyName("trainingComputeBinding")]
    [Required]
    public string TrainingComputeBinding { get; set; }
}
