using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Pandora.Definitions.Attributes;
using Pandora.Definitions.Attributes.Validation;
using Pandora.Definitions.CustomTypes;


// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.


namespace Pandora.Definitions.ResourceManager.Media.v2022_07_01.Encodings;


internal class FiltersModel
{
    [JsonPropertyName("crop")]
    public RectangleModel? Crop { get; set; }

    [JsonPropertyName("deinterlace")]
    public DeinterlaceModel? Deinterlace { get; set; }

    [JsonPropertyName("fadeIn")]
    public FadeModel? FadeIn { get; set; }

    [JsonPropertyName("fadeOut")]
    public FadeModel? FadeOut { get; set; }

    [JsonPropertyName("overlays")]
    public List<OverlayModel>? Overlays { get; set; }

    [JsonPropertyName("rotation")]
    public RotationConstant? Rotation { get; set; }
}
