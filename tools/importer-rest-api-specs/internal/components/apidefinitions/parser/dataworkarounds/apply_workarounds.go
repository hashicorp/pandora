...
// ApplyWorkarounds goes through and determines if any workarounds are required for the Service/API Version
...
func ApplyWorkarounds(input sdkModels.Service) (*sdkModels.Service, error) {
	logging.Debugf("Applying Data Workarounds to the Service %q..", input.Name)
	apiVersions := make(map[string]sdkModels.APIVersion)
	for _, apiVersion := range input.APIVersions {
		logging.Debugf("Applying Data Workarounds to the API Version %q..", apiVersion.APIVersion)
		for _, fix := range workarounds {
			if !fix.IsApplicable(input.Name, apiVersion) {
				logging.Tracef("Data Workaround %q is not applicable - skipping..", fix.Name())
				continue
			}

			logging.Tracef("Applying Data Workaround %q..", fix.Name())
			updated, err := fix.Process(apiVersion)
			if err != nil {
				return nil, fmt.Errorf("applying Swagger Data Workaround %q to Service %q / API Version %q: %+v", fix.Name(), input.Name, apiVersion.APIVersion, err)
			}
			apiVersion = *updated
			logging.Tracef("Applying Data Workaround %q - Completed", fix.Name())
		}
		apiVersions[apiVersion.APIVersion] = apiVersion
		logging.Debugf("Applying Data Workarounds to the API Version %q - Completed", apiVersion.APIVersion)
	}
	input.APIVersions = apiVersions
	return &input, nil
}
