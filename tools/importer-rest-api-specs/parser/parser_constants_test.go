...
func TestParseConstantsIntegersTopLevelAsInts(t *testing.T) {
	...
	if favouriteTable.FieldType != models.Integer {
		t.Fatalf("expected resource.Constants['FavouriteTable'].FieldType to be 'Integer' but got %q", favouriteTable.FieldType)
	}
...
func TestParseConstantsIntegersTopLevelAsStrings(t *testing.T) {
	...
	if favouriteTable.FieldType != models.String {
		t.Fatalf("expected resource.Constants['FavouriteTable'].FieldType to be 'String' but got %q", favouriteTable.FieldType)
	}
	if len(favouriteTable.Values) != 3 {
		t.Fatalf("expected resource.Constants['FavouriteTable'] to have 3 values but got %d", len(favouriteTable.Values))
	}
