...
import (
	...
	"github.com/hashicorp/pandora/tools/generator-agent-docs/internal/cmd"
...
func run() error {
	...
	hclog.Default().SetLevel(loggingOpts.Level)

	c := cli.NewCLI("generator-agent-docs", "1.0.0")
	c.Args = args
...
func parseSourceDataType(args []string) (*sdkModels.SourceDataType, error) {
	if len(args) == 0 {
		return nil, fmt.Errorf("a source data type should be specified as the first argument e.g. `%s`", sdkModels.ResourceManagerSourceDataType)
	}
	rawVal := args[0]
