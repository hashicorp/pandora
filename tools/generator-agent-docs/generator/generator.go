...
// GeneratedResource represents the structured output of the generator.
...
// Generate reads the API definitions and returns a map of file path to GeneratedResource.
func Generate(opts GenerateOptions) (map[string]GeneratedResource, error) {
	if opts.Logger == nil {
	...
	result := make(map[string]GeneratedResource)
	...
	for serviceName, service := range *servicesMap {
		for version, apiVersion := range service.APIVersions {
			for resourceName, apiResource := range apiVersion.Resources {
				...
				// Make a copy of apiResource because we are taking its address
				apiResCopy := apiResource
				
				content := generateResourceMarkdown(serviceName, provider, version, resourceName, apiResource)
				result[path] = GeneratedResource{
					Provider:    provider,
					Service:     serviceName,
					Version:     version,
					Resource:    resourceName,
					Markdown:    string(content),
					APIResource: &apiResCopy,
				}
			}
		}
	}
