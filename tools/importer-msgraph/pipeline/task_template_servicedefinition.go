...
func (pipelineTask) templateServiceDefinitionForService(files *Tree, serviceName, apiVersion string, logger hclog.Logger) error {
	filename := fmt.Sprintf("Pandora.Definitions.%[2]s%[1]s%[3]s%[1]sServiceDefinition.cs", string(os.PathSeparator), definitionsDirectory(apiVersion), serviceName)
	if err := files.addFile(filename, templateServiceDefinition(serviceName, apiVersion)); err != nil {
		return err
	}

	filename = fmt.Sprintf("Pandora.Definitions.%[2]s%[1]s%[3]s%[1]sServiceDefinition-GenerationSettings.cs", string(os.PathSeparator), definitionsDirectory(apiVersion), serviceName)
