	...
	for serviceName, serviceDetails := range result.Services {
		...
		for resourceName, resourceDetails := range serviceDetails.Terraform.Resources {
			...
			*/
			...
			helper := mappings2.NewResourceMappings(resourceDetails, apiResourceDetails.Schema.Constants, apiResourceDetails.Schema.Models)
			for _, item := range resourceDetails.Mappings.ModelToModels {
				mappingsForThisModel, err := mappings2.FindMappingsBetween(item, resourceDetails.Mappings.Fields)
				if err != nil {
